      ...
      <div className="navbar md:container">
            ...
            {navs.map((data, idx) => {
              return (
                <div key={idx}>
                  <Link
                    href={data.path}
                    className={navClass(data.path) + " tab-bordered"}
                  >
                    {data.name}
                  </Link>
                </div>
        ...
        <div className="flex-none md:hidden">
          <div className="dropdown dropdown-end">
            ...
            <ul
              tabIndex={0}
              className="dropdown-content z-[1] tabs tabs-boxed shadow bg-base-100 rounded-box"
            >
              {navs.map((data, idx) => {
                return (
                  <li key={idx}>
                    <Link href={data.path} className={navClass(data.path)}>
                      {data.name}
                    </Link>
                  </li>
                );
